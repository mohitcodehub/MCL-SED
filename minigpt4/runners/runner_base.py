...
def worker_init_fn(worker_id):
    """
    为 DataLoader 的每个 worker 初始化随机种子。
    确保每个 worker 使用一个从全局种子派生出的唯一且确定的种子。
    """
    base_seed = torch.initial_seed() % (2**32 - 1) # 从主进程RNG派生一个种子
...
class RunnerBase:
    ...
    @property
    def scaler(self):
        # bfloat16 training does not require GradScaler and causes scale underflow issues.
        return None
    ...
    @property
    def log_freq(self):
        log_freq = self.config.run_cfg.get("log_freq", 500)
        return int(log_freq)
            ...
            if isinstance(dataset, ChainDataset) or isinstance(
                ...
                if self.use_distributed:
                    sampler = DistributedSampler(
                        ...
                        rank=get_rank(),
                        seed=torch.initial_seed() + get_rank(),
                    )
