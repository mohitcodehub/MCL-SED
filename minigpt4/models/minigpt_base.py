...
class MiniGPTBase(BaseModel):
    ...
    def get_context_emb(self, prompt, img_list):
        ...
        seg_tokens = [
            self.llama_tokenizer(
                seg, return_tensors="pt", add_special_tokens=False).to(device).input_ids # don't add bos here
    ...
    def forward(self, samples, reduction='mean'):
        ...
        if "audio_features" not in samples:
    ...
    def generate(
        ...
        img_embeds, atts_img = self.encode_img(images.to(self.device), video_features.to(self.device), audio_features.to(self.device))
        ...
        # Build generation kwargs — top_p is only valid with do_sample=True
        gen_kwargs = dict(
            inputs_embeds=embs,
